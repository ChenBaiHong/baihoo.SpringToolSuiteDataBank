package com.baihoo.blog.db.mysqldb.runner;
...
import com.baihoo.blog.db.mysqldb.DbUtils;
...
public class QueryRunner {
	...
	public Integer insertBean(Connection connection , Object  bean) throws Exception {
			Map<String , String> map = DbUtils.getTableValInMap(bean);
			//字段注解对象
	...
	****************************************************************************************/
	...
	static class HandleBean implements HandleResultSet{
		...
		public HandleBean(Class  clazz){
			this.clazz=clazz;
			classFields = DbUtils.classFields(clazz);
			try {
				claOraMap = DbUtils.classTableFieldInMap(clazz);
				oraClaMap = DbUtils.tableClassFieldInMap(clazz);
			}catch(Exception e) {
				e.printStackTrace();
			}
		}
